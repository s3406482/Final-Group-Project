    ...
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                ...
                <th scope="col"><?= $this->Paginator->sort('developer') ?></th>
                <th scope="col"><?= $this->Paginator->sort('date_closed') ?></th>
                
            </tr>
        </thead>
        <tbody>
            <?php foreach ($recommendedJobs as $jobId=>$recommendedDevelopers): ?>
            <?php foreach ($recommendedDevelopers as $developerId): ?>
            <tr>
                <td><?= $this->Html->link($jobTitles[$jobId], ['action' => 'view', $jobId]) ?></td>
                <td class="show-for-large-up"><?= substr(h($job->description[$jobId]),0,150) ?></td>
                <td><?= $this->Html->link($employerNames[$jobId][1], ['controller' => 'Employers', 'action' => 'view', $employerNames[$jobId][0]])?></td>
                <td><?= h($developerNames[$developerId]) ?></td>
                <td><?= h($jobDateClosed[$jobId]->format('j F Y')) ?></td>
            </tr>
            <?php endforeach; ?>
            <?php endforeach; ?>
        </tbody>
